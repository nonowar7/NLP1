...
class ExtractFeaturesNER:
    ...
    def readInputFile(self, file_name):
        ...
        lines = []
        for line in content:
            new_line = "STARTword/STARTtag STARTword/STARTtag "\
                       + line + " ENDword/ENDtag ENDword/ENDtag"
            lines.append(new_line)
        return lines

    def writeFeaturesToFile(self, features_file):
    ...
    def getMostCommonTagForWord(self, train_data):
        vec = CountVectorizer(lowercase=False, token_pattern=r"\S+", min_df=1)
        values = vec.fit_transform(train_data).sum(axis=0).A1
        names = vec.get_feature_names()
        ...
        for j, token in enumerate(sent[i:i + 2]):
            ...
            if j == 0 and rare:
                ...
                for k in range(min(4, math.floor(len_tok / 2))):
                    ...
                    features = " ".join(
                        [features, '='.join([''.join(['SC_', str(k)]), token[len_tok - 1 - k:len_tok]])])
    ...
    def createFeaturesDict(self, features_count, known_words):
        for entry in features_count:
            ...
            self.features_list.append(feature_str)
    ...
    def runExtractFeatures(self):
        ...
        self.getMostCommonTagForWord(train_data)
        self.createFeaturesDict(features_count, known_words)
        self.writeFeaturesToFile(features_file)
        print(time.time()-a)


def main():
    ef = ExtractFeaturesNER()
